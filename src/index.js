        ...
        return(
            <div className="entry">
                <fieldset>
                    ...
                    <div>金額： <input type="text" value={this.state.amount} onChange={(event) => this.onChangeAmount(event)} placeholder="3000" /></div>
    ...
    return(
        <div>
            <table className="book">
                ...
                <tbody>
                    {props.books.map((book) => <MoneyBookItem book={book} key={book.date + book.item} />)}
                </tbody>
            </table>
        </div>
    )
